...
addGeoJSON <- function(map, geojson, layerId = NULL, group = NULL,
  ...
  noClip = FALSE,
  options = pathOptions(),
  hoverHighlightOptions=NULL,
  data = getMapData(map)
) {
  options <- c(options, filterNULL(list(
    stroke = stroke, color = color, weight = weight, opacity = opacity,
    fill = fill, fillColor = color, fillOpacity = fillOpacity,
    dashArray = dashArray, smoothFactor = smoothFactor, noClip = noClip
  )))
  invokeMethod(map, data, "addGeoJSON", geojson, layerId, group,options,hoverHighlightOptions)
}
